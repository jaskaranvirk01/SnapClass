...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button('Add Photos', type='primary',
                     icon=':material/photo_prints:', width='stretch'):
            add_photo_dialog()
        ...
        if st.button('Run Face Analysis', width='stretch', type='secondary', icon=':material/analytics:', disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos...'):
                ...
                enrolled_res = supabase.table(
                    'subject_students').select('*, students(*)').eq('subject_id', selected_subject_id).execute()
...
def teacher_tab_attendance_records():
    ...
    summary['Attendance Stats'] = (
        '✅' +
        summary['Present_Count'].astype(
            str) + ' / ' + summary['Total_Count'].astype('str') + ' Students '
    )

    display_df = (summary.sort_values(
        by='ts_group', ascending=False)[['Time', 'Subject', 'Subject Code', 'Attendance Stats']])
